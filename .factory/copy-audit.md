# Copy audit — 2026-08-28, polish 5

Commands, option names, terminal output, the product name, and the decorative
check mark are excluded from sentence counts. Counts are whitespace-separated
tokens, including visible option names. `npm run audit:copy` checks every row
against the built pages or README and recomputes every recorded count. Every
visitor-facing sentence is 22 words or fewer. No banned marketing wording appears.
...
| Check before changing cloud data. | 5 | Clear image caption |
...
## Demo and shared routes

| Text | Words | Audit |
| --- | ---: | --- |
| Demo — sample data, nothing is saved | 7 | Claim: `browser-demo-isolated` |
| Reset demo | 2 | Clear reset control |
| Start for real | 3 | Clear exit control |
| Bundled sample project | 3 | Clear label |
| This sample selects the wrong Firebase project. | 7 | Clear demo h1 |
| Sample result | 2 | Clear result label |
| CAUTION · Wrong project selected | 5 | Generated demo result |
| Selected project | 2 | Clear result label |
| Project file default | 3 | Clear result label |
| Next check | 2 | Clear result label |
